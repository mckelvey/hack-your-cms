...
$(document).ready ->
  ...
  sendToDisplay = ->
    ...
    <article id="' + item.id + '" style="opacity: 0;left:' + (screenWidth * shown + 18) + 'px;width: ' + (screenWidth-36) + 'px;height: ' + (screenHeight-36) + 'px;">
      ' + (if item.images? and item.images[0]? then '<img src="' + item.images[0].url.replace(/^.*\/([^\/]+)$/i, '/prototype-local/events/$1') + '" alt="' + item.images[0].alt + '" />' else '') + '
      <div>
  ...
  buffer = ->
    log 'buffering...'
    $.ajax({
      url: "http://localhost:1969/prototype-local/events/index.json"
      method: 'GET'
      dataType: 'json'
      success: (data, textStatus, jqXHR) ->
        log 'received data'
